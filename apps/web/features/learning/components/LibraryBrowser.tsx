...
const labels: Record<LearningCategory | 'all', string> = {
  all: 'Tất cả',
  vocabulary: 'Từ vựng',
  grammar: 'Ngữ pháp',
  reading: 'Đọc',
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-1 overflow-x-auto">
          {(Object.keys(labels) as (LearningCategory | 'all')[]).map((key) => (
            <button
              key={key}
              type="button"
              onClick={() => setCategory(key)}
              className={
                category === key ? 'btn-primary h-9 px-3 text-sm' : 'btn-ghost h-9 px-3 text-sm'
              }
            >
              {labels[key]}
            </button>
        ...
        {results.map((item) => {
          ...
          return (
            <Link
              key={item.category}
              href={item.href}
              className="hover:bg-muted/60 group bg-card p-5 transition-colors sm:p-6"
            >
              ...
              <p className="mt-5 text-sm font-medium text-primary">
                {item.itemCount} {item.category === 'listening' ? 'buổi đã ghi' : 'bộ tài liệu'}
              </p>
            </Link>
          );
        })}
      ...
      {results.length === 0 && (
        <p className="card p-8 text-center text-sm text-muted-foreground">
          Chưa có tài liệu phù hợp. Thử một từ khóa hoặc nhóm khác.
        </p>
      )}
    </div>
  );
